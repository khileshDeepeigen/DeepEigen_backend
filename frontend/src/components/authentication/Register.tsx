...
const Register: React.FC = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            // ✅ Using csrfFetch wrapper: automatically includes CSRF token + credentials
            const response = await csrfFetch("http://localhost:8000/accounts/register/", {
                method: "POST",
                body: JSON.stringify({
            ...
            <div className="flex-1 flex flex-col justify-center p-4 sm:p-6 md:p-8 lg:p-12 xl:p-20 bg-white">
                <div className="w-full max-w-md sm:max-w-lg md:max-w-2xl mx-auto">
                    ...
                    <form className="space-y-4 sm:space-y-6" onSubmit={handleSubmit}>
                        ...
                        {/* Success Message */}
                        {successMsg && (
                            <div className="w-full bg-green-50 border border-green-300 text-green-700 px-4 py-3 rounded-lg text-sm">
                                {successMsg}
                            </div>
                        )}
